...
- 先画层级图：domain → infrastructure → application → interfaces
- 用"餐厅"类比：interfaces=服务员、application=厨师长、domain=菜谱和食材标准、infrastructure=厨房设备供应商
- 强调"依赖方向内层不依赖外层"的核心规则
- **特别说明**：找到一个从 API 接口到数据库的完整调用链，跟着走一遍就懂了
...
### Agent / AI 应用项目
- 先理解 Agent 的四种路由模式（Chat/Tool/RAG/ReAct）
- 画出 LLM → 记忆层 → RAG引擎 → 工具执行 → 沙箱 的数据流
- **特别说明**：Schema 驱动的提示词组装是核心——理解 RuntimeContext 如何填充 Slot
- **实操**：追踪一次 `/api/chat` 请求从 FastAPI handler 到 UnifiedAgent 的完整流程
...
- **Python/FastAPI 生态术语优先使用 references 中的翻译**
